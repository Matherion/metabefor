...
- `list`: If multiple closely related entities are extracted (e.g. effect sizes), it is often more efficient to extract these in a entity list than by creating a single extractable entity for each entity. By setting `list` to `TRUE`, this can be realised.
- `collapsing`: 
- `repeating`:
